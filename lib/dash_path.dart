...
Path dashPath(Path source, {
  ...
  for (final PathMetric metric in source.computeMetrics()) {
    double distance = dashOffset._calculate(metric.length) * animationValue;
    bool draw = true;
    while (distance < metric.length) {
      final double len = dashArray.next;
      if (draw) {
        dest.addPath(metric.extractPath(distance * animationValue, (distance + len)* animationValue), Offset.zero);
      }
      distance += len;
      draw = !draw;
    }
